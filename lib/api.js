...
module.exports = {
  ...
  extractPlaylist: async function (playlistId) {
    ...
    spinner.info('extracting playlist tracks ' +
      `${tracks.length}/${data.tracks.total}`);
    if (data.tracks.next) {
      ...
      while (tracks.length < data.tracks.total) {
        ...
        spinner.info('extracting playlist tracks ' +
          `${data.tracks.length}/${data.tracks.total}`);
      }
    }
  ...
  extractAlbum: async function (albumId) {
    ...
    spinner.info('extracting album tracks ' +
      `${tracks.length}/${data.tracks.total}`);
    if (data.tracks.next) {
      let offset = tracks.length;
      while (tracks.length < data.tracks.total) {
  ...
  extractArtistAlbums: async function (artistId) {
    ...
    spinner.info('extracting albums ' +
      `${albums.length}/${artistAlbums.total}`);
    if (artistAlbums.next) {
      ...
      while (albums.length < artistAlbums.total) {
        ...
        offset += MAX_LIMIT_DEFAULT;
        spinner.info('extracting albums ' +
          `${artistAlbums.total}/${artistAlbums.total}`);
      }
    }
  ...
  extractSavedAlbums: async function () {
    ...
    spinner.info('extracting albums ' +
      `${albums.length}/${savedAlbums.total}`);
    if (savedAlbums.next) {
      let offset = albums.length;
      while (albums.length < savedAlbums.total) {
        ...
        offset += MAX_LIMIT_DEFAULT;
        spinner.info('extracting albums ' +
          `${savedAlbums.length}/${savedAlbums.total}`);
      }
    }
  ...
  extractSavedPlaylists: async function () {
    ...
    spinner.info('extracting playlists ' +
      `${playlists.length}/${savedPlaylists.total}`);
    if (savedPlaylists.next) {
      ...
      while (playlists.length < savedPlaylists.total) {
        ...
        offset += MAX_LIMIT_DEFAULT;
        spinner.info('extracting playlists ' +
          `${savedPlaylists.length}/${savedPlaylists.total}`);
      }
    }
