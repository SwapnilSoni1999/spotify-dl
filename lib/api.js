...
const {
  ...
  INPUT_TYPES,
  MAX_LIMIT_DEFAULT,
  SERVER: { HOST, CALLBACK_URI, TOKEN_URI },
} = Constants;

const redirectUri = `https://${HOST}${CALLBACK_URI}`;
const tokenUri = new URL(TOKEN_URI, `https://${HOST}`).toString();
...
const parseSpotifyCredentials = appKey => {
  ...
  if (!clientId || !clientSecret) {
    logFailure(
      'Invalid app key format. Use --app-key "<client-id>:<client-secret>"'
    );
    process.exit(1);
  }
...
const getSpotifyApi = () => {
  ...
  const { appKey } = cliInputs();
  const { clientId, clientSecret } = parseSpotifyCredentials(appKey);
  spotifyApi = new Spotified({
    clientId,
    clientSecret,
  });
...
const getCode = async state => {
  const searchParams = new URLSearchParams({ state });
  const getCodeUri = `${tokenUri}?${searchParams.toString()}`;
  let lastError;
  await sleep(3000);
  try {
    const response = await fetch(getCodeUri);
    const responseBody = await response.text();

    if (!response.ok) {
      if (response.status == 400) {
        throw new Error(`Token endpoint rejected state: ${responseBody || 'Invalid state'}`);
      }

      throw new Error(
        `Token endpoint returned ${response.status}: ${responseBody || 'No response body'}`
      );
    }

    let code;
    try {
      code = JSON.parse(responseBody).code;
    } catch {
      code = responseBody;
    }
    ...
    lastError = new Error('Token endpoint response did not include a code value.');
  } catch (error) {
    if (error.message.startsWith('Token endpoint rejected state')) {
      throw error;
    }
  ...
  throw new Error(
    `Timed out waiting for auth code from ${tokenUri}. ${lastError ? lastError.message : ''}`.trim()
  );
};
...
const requestAuthorizedTokens = async () => {
  ...
  logInfo('waiting for auth to complete... Please Wait...');
  const code = await getCode(state);
  setTokens(
