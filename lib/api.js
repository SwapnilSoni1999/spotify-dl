...
export async function extractEpisodes(episodeIds) {
  ...
  for (let x = 0; x < chunkedEpisodes.length; x++) {
    ...
    episodesResult = await callSpotifyApi(
      async () =>
        (
          await spotifyApi.getEpisodes(chunkedEpisodes[x])
        ).body.episodes,
    );
...
export async function extractTracksFromData(
  apiCall,
  limit = MAX_LIMIT_DEFAULT,
) {
  const tracks = [];
  ...
  let data;
  do {
    data = await callSpotifyApi(apiCall);
    ...
    tracks.push(...items.map(item => parseTrack(item.track, audioFeatures)));
    offset += limit;
    logInfo('extracting tracks ' + `${tracks.length}/${data.total}`);
  } while (tracks.length < data.total);
