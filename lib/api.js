...
const authSetupMessage =
  ...
  `if you get a 404, please add the following line to ${process.platform === 'win32' ? 'C:\\Windows\\System32\\drivers\\etc\\hosts' : '/etc/hosts'}: \n` +
  `127.0.0.1 ${HOST}`;

const getHostsFilePaths = () => {
  if (process.platform === 'win32') {
    const windowsRoot = process.env.SystemRoot || process.env.windir || 'C:\\Windows';

    return [
      `${windowsRoot}\\System32\\drivers\\etc\\hosts`,
      `${windowsRoot}\\System32\\Drivers\\etc\\hosts`,
    ];
  }
...
const hasHostEntry = host => {
  ...
  try {
    for (const hostsFilePath of getHostsFilePaths()) {
      try {
        const hostsFileContent = fs.readFileSync(hostsFilePath, 'utf8');
        if (hostEntryPattern.test(hostsFileContent)) {
          return true;
        }
      } catch {
        continue;
